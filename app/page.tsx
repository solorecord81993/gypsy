...
function answerByContext(score: number, kind: QuestionKind) {
  ...
  const cautious: Record<QuestionKind, string> = {
    love: "ความสัมพันธ์นี้ยังไปต่อได้ แต่ควรคุยให้ชัดว่าทั้งสองฝ่ายคาดหวังอะไรและจุดใดไม่ตรงกัน",
    work: "เรื่องงานนี้พอไปต่อได้ แต่ยังไม่ควรทุ่มเต็มกำลังจนกว่าจะตรวจรายละเอียดและผู้เกี่ยวข้องให้ครบ",
    ...
    decision: "ทางเลือกนี้ยังไม่ควรตอบตกลงทันที ควรเขียนข้อดี ข้อเสีย และภาระที่ต้องรับให้ครบก่อน",
    general: "เรื่องที่ถามยังไปต่อได้ แต่ควรระบุสิ่งที่ทำให้ติดขัดและเลือกแก้ทีละข้อ",
  };
...
function actionableStep(card: DrawnCard, kind: QuestionKind) {
  if (card.id === "major-15") {
    const devilSteps: Record<QuestionKind, string> = {
      love: "บอกให้ชัดว่าความกลัว ความหึง หรือความยึดติดใดกำลังทำให้ความสัมพันธ์ไม่เป็นอิสระ แล้วตั้งขอบเขตหนึ่งข้อ",
      work: "เขียนให้ชัดว่าแรงกดดัน บุคคล หรือผลประโยชน์ใดทำให้คุณเปลี่ยนวิธีทำงานไม่ได้ แล้วเลือกหนึ่งข้อที่ลดหรือตัดออกได้",
      money: "แยกหนี้ ภาระ และความอยากที่กำลังบังคับการตัดสินใจ แล้วหยุดรายการที่ไม่จำเป็นหนึ่งรายการ",
      health: "ระบุพฤติกรรมที่รู้ว่าไม่ดีแต่ยังหยุดไม่ได้ แล้วเริ่มลดพฤติกรรมนั้นอย่างวัดผลได้",
      decision: "เขียนข้อผูกมัดหรือผลประโยชน์ที่ทำให้ตัดสินใจไม่เป็นอิสระ แล้วตัดตัวเลือกที่รับผลเสียไม่ไหว",
      general: "เขียนให้ชัดว่าสิ่งใดกำลังควบคุมการตัดสินใจของคุณ แล้วเลือกหนึ่งอย่างที่ลดอิทธิพลได้ทันที",
    };
    return devilSteps[kind];
  }
  const majorSteps: Record<string, string> = {
    "major-00": "ตรวจว่ามีข้อมูล คนช่วย และทางถอยพร้อมหรือไม่ แล้วทดลองก้าวแรกในขนาดเล็กก่อน",
    "major-01": "เขียนทรัพยากรที่มีอยู่สามข้อ แล้วเลือกหนึ่งข้อมาใช้สร้างผลงานชิ้นแรก",
    "major-02": "จดข้อมูลที่ยังไม่รู้ แล้วถามผู้เกี่ยวข้องให้ได้คำตอบก่อนตัดสินใจ",
    "major-03": "กำหนดเวลาและทรัพยากรที่จะดูแลเรื่องนี้ในแต่ละสัปดาห์ให้ชัด",
    "major-04": "กำหนดว่าใครรับผิดชอบอะไรและต้องเสร็จเมื่อไร แล้วแจ้งให้ผู้เกี่ยวข้องรับรู้ตรงกัน",
    "major-05": "นำคำถามนี้ไปขอความเห็นจากคนที่มีประสบการณ์ตรงหนึ่งคน แล้วเทียบกับเงื่อนไขของคุณ",
    "major-06": "เขียนหลักสำคัญที่คุณยอมแลกไม่ได้สามข้อ แล้วเลือกทางที่ตรงกับหลักเหล่านั้นมากที่สุด",
    "major-07": "เลือกเป้าหมายเดียวและเขียนสามขั้นตอนถัดไป พร้อมกำหนดวันตรวจความคืบหน้า",
    "major-08": "หยุดก่อนตอบโต้ แล้วคุยด้วยข้อเท็จจริงทีละประเด็นแทนการใช้อารมณ์",
    "major-09": "เว้นระยะหนึ่งวัน แล้วเขียนข้อเท็จจริงสามข้อก่อนกลับมาตัดสินใจ",
    "major-10": "เตรียมแผนหลักและแผนสำรองอย่างละหนึ่งแผน เพื่อรับมือเมื่อสถานการณ์เปลี่ยน",
    "major-11": "รวบรวมหลักฐาน ค่าใช้จ่าย และข้อตกลงให้ครบ แล้วใช้เกณฑ์เดียวกันเปรียบเทียบทุกทางเลือก",
    "major-12": "หยุดวิธีที่ฝืนอยู่ชั่วคราว แล้วถามคนอีกหนึ่งคนว่ามีวิธีทำเรื่องนี้ต่างออกไปอย่างไร",
    "major-13": "เขียนหนึ่งสิ่งที่ต้องหยุดหรือปิดให้จบ แล้วกำหนดวันเริ่มวิธีใหม่",
    "major-14": "ลดขอบเขตงานลงหนึ่งส่วน แล้วกำหนดจังหวะที่ทำต่อเนื่องได้โดยไม่หมดแรง",
    "major-16": "เก็บสิ่งจำเป็นและสำรองข้อมูลก่อน แล้วหยุดใช้แผนที่ตั้งอยู่บนข้อสมมติซึ่งยังไม่ได้ตรวจ",
    "major-17": "ตั้งเป้าหมายเล็กที่ทำเสร็จได้ในหนึ่งสัปดาห์ แล้ววัดผลก่อนขยายต่อ",
    "major-18": "แยกสิ่งที่รู้จริงออกจากสิ่งที่กังวล แล้วตรวจข้อมูลสำคัญกับแหล่งที่เชื่อถือได้สองแหล่ง",
    "major-19": "บอกเป้าหมายและผลงานที่ทำได้ให้ผู้เกี่ยวข้องเห็นอย่างชัดเจน",
    "major-20": "เขียนบทเรียนจากรอบก่อนหนึ่งข้อ แล้วกำหนดว่างานรอบนี้จะทำต่างจากเดิมอย่างไรหนึ่งอย่าง",
    "major-21": "ทำรายการสิ่งที่ยังค้าง แล้วปิดรายการสำคัญให้ครบก่อนเริ่มงานรอบใหม่",
  };
  if (majorSteps[card.id]) return majorSteps[card.id];
  return card.advice;
}
...
function cardReading(card: DrawnCard, position: string, kind: QuestionKind, intent: QuestionIntent) {
  ...
  const action = actionableStep(card, kind);
  ...
  const role = /อุปสรรค|แรงต้าน|กังวล|ต้องระวัง/.test(position)
    ...
    : /คำแนะนำ|ท่าที|แนวทาง/.test(position)
      ? `ทางที่ควรทำคือ ${action} เพื่อแก้หรือใช้ประโยชน์จากภาวะ “${meaning}”`
  ...
  const intentLead: Record<QuestionIntent, string> = {
    yesno: score >= .55 ? "คำตอบจากไพ่ใบนี้: ค่อนข้างใช่หรือมีโอกาสสูง" : score <= -.45 ? "คำตอบจากไพ่ใบนี้: ยังไม่ใช่หรือไม่ควรในตอนนี้" : `คำตอบจากไพ่ใบนี้: เป็นไปได้ โดยขั้นแรกให้${action}`,
    when: score >= .55 ? "ไพ่ใบนี้บอกว่าเรื่องมีโอกาสขยับในระยะใกล้" : score <= -.45 ? `ไพ่ใบนี้บอกว่ายังไม่เกิดเร็ว ๆ นี้ และขั้นแรกควร${action}` : `ไพ่ใบนี้บอกว่าเรื่องจะขยับหลังจากคุณ${action}`,
    ...
    outcome: score >= .55 ? `คำทำนายตรง ๆ: ${subject}มีแนวโน้มดีขึ้นหรือเดินหน้า` : score <= -.45 ? `คำทำนายตรง ๆ: ${subject}มีแนวโน้มติดขัดหรือไม่เป็นตามหวัง` : `คำทำนายตรง ๆ: ${subject}ยังไปต่อได้ โดยขั้นแรกให้${action}`,
  };
...
function overallReading(cards: DrawnCard[], kind: QuestionKind, intent: QuestionIntent) {
  ...
  const riskAction = actionableStep(risk, kind);
  ...
  const direct: Record<QuestionIntent, string> = {
    yesno: score >= .4 ? "คำตอบตรง ๆ: มีโอกาสเป็นไปได้ค่อนข้างมาก" : score <= -.35 ? `คำตอบตรง ๆ: ยังไม่ควรเดินหน้าตอนนี้ ขั้นแรกให้${riskAction}` : `คำตอบตรง ๆ: เป็นไปได้ โดยขั้นแรกให้${riskAction}`,
    when: score >= .4 ? `คำตอบเรื่องเวลา: ${timing}` : score <= -.35 ? `คำตอบเรื่องเวลา: ยังไม่เกิดเร็ว ๆ นี้ ขั้นแรกให้${riskAction}` : `คำตอบเรื่องเวลา: ${timing} โดยก่อนหน้านั้นควร${riskAction}`,
    ...
    choice: score >= .4 ? "คำตอบตรง ๆ: ทางเลือกนี้มีข้อดีมากกว่าและพอเดินหน้าต่อได้" : score <= -.35 ? `คำตอบตรง ๆ: ยังไม่ควรเลือกทางนี้ ขั้นแรกให้${riskAction}` : `คำตอบตรง ๆ: ทางเลือกนี้พอไปต่อได้ แต่ก่อนตัดสินใจให้${riskAction}`,
    outcome: score >= .4 ? `คำทำนายตรง ๆ: ${subject}มีแนวโน้มคืบหน้าและจบดีขึ้น` : score <= -.35 ? `คำทำนายตรง ๆ: ${subject}มีแนวโน้มติดขัด หากยังทำแบบเดิม ขั้นแรกให้${riskAction}` : `คำทำนายตรง ๆ: ${subject}ยังไปต่อได้ โดยขั้นแรกให้${riskAction}`,
  };
  ...
  const firstStep = cardScore(risk) <= -0.25 ? riskAction : actionableStep(last, kind);
  const lastStep = actionableStep(last, kind);
  const followUp = firstStep !== lastStep ? ` จากนั้นจึง${lastStep}` : "";
